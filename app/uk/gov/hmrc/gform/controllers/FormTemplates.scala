...
import uk.gov.hmrc.gform.models.{ ComponentType, DbOperationResult, FormTemplate, FormTypeId, Schema, Section }
...
object FormTemplates {
  def saveTemplate(
    ...
    // format: OFF
    for {
      _          <- fromOptA          (Section.validateHelpText(sectionsList).toEither)
