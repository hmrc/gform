...
case object Validators {
  private val templateReads: Reads[Validators] = Reads { json =>
  ...
  implicit val format: OFormat[Validators] = OFormatWithTemplateReadFallback(templateReads)

  implicit val leafExprs: LeafExpr[Validators] = (path: TemplatePath, t: Validators) =>
    t match {
