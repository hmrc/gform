...
final case class ExprWithPath(path: TemplatePath, expr: Expr) {
  ...
  def referenceInfos: List[ReferenceInfo] = expr match {
    ...
    case f @ FormCtx(_: FormComponentId)          => ReferenceInfo.FormCtxExpr(path, f) :: Nil
    case s @ Sum(field1: Expr)                    => ReferenceInfo.SumExpr(path, s) :: Nil
    case c @ Count(_: FormComponentId)            => ReferenceInfo.CountExpr(path, c) :: Nil
    case i @ Index(_: FormComponentId)            => ReferenceInfo.IndexExpr(path, i) :: Nil
    ...
    case DateCtx(DataRetrieveDateCtx(id, a))      => ReferenceInfo.DataRetrieveCtxExpr(path, DataRetrieveCtx(id, a)) :: Nil
    case DateCtx(dateExpr)                        => dateExpr.referenceInfos
    case DateFunction(value)                      => ReferenceInfo.DateFunctionExpr(path, value) :: Nil
    case p @ Period(_, _, _)                      => ReferenceInfo.PeriodExpr(path, p) :: Nil
    ...
    case s @ IndexOfInChoice(_, _)                => ReferenceInfo.IndexOfInChoiceExpr(path, s) :: Nil
    case IndexOfDataRetrieveCtx(ctx, expr)        => ReferenceInfo.DataRetrieveCtxExpr(path, ctx) :: toReferenceInfo(expr)
    ...
    case c @ ChoicesAvailable(_, _)               => ReferenceInfo.ChoicesAvailableExpr(path, c) :: Nil
    case s @ CountSelectedChoices(_)              => ReferenceInfo.CountSelectedChoicesExpr(path, s) :: Nil
    case c @ ChoicesCount(_)                      => ReferenceInfo.ChoicesCountExpr(path, c) :: Nil
    case t @ TaskStatus(_)                        => ReferenceInfo.TaskStatusExpr(path, t) :: Nil
    case c @ DisplayAsEntered(_)                  => ReferenceInfo.DisplayAsEnteredExpr(path, c) :: Nil
