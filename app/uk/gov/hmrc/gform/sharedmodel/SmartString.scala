    ...
    jsObjects.reverse.filterNot {
      ...
      case lastObject :: ifObjects =>
        ...
        if ((lastObject \ "includeIf").isDefined) {
          JsError("An array of objects for a smartString must have at least 1 object without an includeIf")
        } else {
          val ifConditions = ifObjects.reverse.map(jsObj =>
            for {
              b        <- getBooleanExpr(jsObj)
              internal <- Json.fromJson[SmartStringInternal](jsObj - "includeIf")
            } yield (b, internal)
          )
          if (ifConditions.forall(_.isSuccess)) {
            ...
            for {
              last1 <- last
            } yield SmartStringCond(ifConditions1, last1)
          } else {
            JsError(ifConditions.collectFirst { case JsError(errors) => errors }.getOrElse(Seq()))
          }
        }
      case _ => JsError("There are no elements in the conditional SmartString")
    }
  }

  private def getBooleanExpr(jsObj: JsObject): JsResult[BooleanExpr] =
    (jsObj \ "includeIf").asOpt[JsString] match {
      case None => JsError("Failed to parse includeIf: includeIf is missing")
      case Some(includeIf) =>
        booleanExprParser(includeIf) match {
          case JsError(unexpectedState) =>
            JsError(s"Failed to parse boolean expr from  includeIf jso: ${unexpectedState.toString}")
