...
class ShutterFormTemplateRepository(mongoModule: MongoModule)(implicit ec: ExecutionContext)
    extends PlayMongoRepository[ShutterFormTemplate](
      collectionName = "shutterFormTemplate",
      mongoComponent = mongoModule.mongoComponent,
      domainFormat = ShutterFormTemplate.format,
      indexes = Seq(
        IndexModel(
          Indexes.ascending("shutterMessageId"),
          IndexOptions()
            .background(true)
            .name("shutterMessageId")
        )
      )
    ) {
  ...
  def findByShutterMessageId(shutterMessageId: ShutterMessageId): Future[List[ShutterFormTemplate]] =
    collection
      .find(Document("shutterMessageId" -> shutterMessageId.value))
      .toFuture()
      .map(_.toList)
  ...
  def upsert(shutterFormTemplate: ShutterFormTemplate): Future[ShutterFormTemplate] =
    collection
      .findOneAndReplace(
        Document("_id" -> shutterFormTemplate._id.value),
        shutterFormTemplate,
