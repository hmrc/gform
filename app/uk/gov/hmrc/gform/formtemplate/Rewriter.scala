...
import uk.gov.hmrc.gform.sharedmodel.formtemplate.destinations.Destination.{ DataStore, Email, HandlebarsHttpApi, HmrcDms, InfoArchive, Log, PegaApi, StateTransition, SubmissionConsolidator }
  ...
  private def validateAndRewriteBooleanExprs(formTemplate: FormTemplate): Either[UnexpectedState, FormTemplate] = {
    ...
    val fcLookup: Map[FormComponentId, ComponentType] =
      formTemplate.formKind.allSections.foldLeft(fcLookupDeclaration) {
        ...
        case (
              acc,
              Section.AddToList(_, _, _, _, _, _, _, _, pages, _, _, _, _, _, _, _, _, _, _, _, _, _, maybeDec, _)
            ) =>
    ...
    val pages: List[Page] = formTemplate.formKind.allSections.flatMap {
      ...
      case Section.AddToList(_, _, _, _, _, _, _, _, pages, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _) => pages.toList
    }
          ...
          .fold[Either[UnexpectedState, BooleanExpr]](missingFormComponentId(formComponentId)) {
            ...
            case Choice(_, options, _, _, _, _, _, _, _, _, _) =>
              ...
              val exprBasedExists = options.collectFirst {
                case OptionData.ValueBased(_, _, _, _, OptionDataValue.ExprBased(_), _, _) =>
              }.isDefined
            ...
            case RevealingChoice(options, _) =>
              val possibleValues = options
                .map(_.choice)
                .collect { case OptionData.ValueBased(_, _, _, _, OptionDataValue.StringBased(value), _, _) =>
                  value
                }
                .toSet
              ...
              val exprBasedExists = options
                .map(_.choice)
                .collectFirst { case OptionData.ValueBased(_, _, _, _, OptionDataValue.ExprBased(expr), _, _) =>
                  expr
                }
                .isDefined
      ...
      def replaceFormComponentNested(formComponent: FormComponent): FormComponent = formComponent match {
        ...
        case IsChoice(choice) =>
          replaceFormComponent(formComponent).copy(`type` = choice.copy(options = choice.options.map {
            case OptionData.ValueBased(l, h, i, d, v, s, k) =>
              OptionData.ValueBased(l, h, replaceIncludeIf(i), d, v, s, k)
