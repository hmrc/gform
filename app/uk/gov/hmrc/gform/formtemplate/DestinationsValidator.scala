...
object DestinationsValidator {
  ...
  def validateTestDestinationIdsExist(destinations: Destinations): ValidationResult =
    destinations match {
      case destinationList: Destinations.DestinationList => validateDestinationListTestReferences(destinationList)
      case _: Destinations.DmsSubmission                 => Valid
    }

  private def validateDestinationListTestReferences(destinations: Destinations.DestinationList): ValidationResult = {
    val destinationIds = destinations.destinations.map(_.id).toList.toSet
    val referencedDestinationIds = destinations.tests.flatMap(_.expectedResults.map(_.destinationId)).toSet
  ...
  private def validateTests(template: FormTemplate, dl: Destinations.DestinationList) = {
  ...
  def validate(template: FormTemplate): Opt[Unit] =
    for {
      ...
      _ <- validateTestDestinationIdsExist(template.destinations).toEither
