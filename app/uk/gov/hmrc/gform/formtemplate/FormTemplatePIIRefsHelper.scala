...
object FormTemplatePIIRefsHelper {
  ...
  private def extractRefs(expr: Expr): List[String] =
    expr match {
      ...
      case Period(dateCtx1, dateCtx2, _)                          => extractRefs(dateCtx1) ++ extractRefs(dateCtx2)
      ...
      case NumberedListChoicesSelected(FormComponentId(value), _) => List(value)
      case BulletedListChoicesSelected(FormComponentId(value), _) => List(value)
      ...
      case LookupOps(expr, _)                                     => extractRefs(expr)
    }
