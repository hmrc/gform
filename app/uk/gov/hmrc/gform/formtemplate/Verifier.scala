...
trait Verifier {
  def verify(
    ...
    for {
      ...
      _ <- fromOptA(FormTemplateValidator.validateDataRetrieveBlockAttributes(formTemplate, pages).toEither)
      ...
      _ <- fromOptA(FormTemplateValidator.validateConfirmations(pages, expressionIds).toEither)
