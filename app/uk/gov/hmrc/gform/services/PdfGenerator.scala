...
import uk.gov.hmrc.gform.models.{ Choice, FieldId, FormField, SectionFormField }
...
object PDFBoxExample {
  ...
  def jsonParser(json: JsObject): ArrayBuffer[String] = {
    ...
    for (elem <- fields) {
      if (elem.id == FieldId("environmentalBodies")) {
...
object SPDFExample {
  def generateSpdf(form: String) = {
    ...
    val page = uk.gov.hmrc.gform.views.html.summary.render(data).body
