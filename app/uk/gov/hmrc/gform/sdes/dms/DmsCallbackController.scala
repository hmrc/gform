...
class DmsCallbackController(
  ...
  def callback: Action[CallBackNotification] = Action.async(parse.json[CallBackNotification]) { implicit request =>
    ...
    logger.info(
      s"SDES: Received dms callback for fileName: $fileName, correlationId : $correlationID, status : $responseStatus and failedReason: ${responseFailureReason
        .getOrElse("")} "
    )
