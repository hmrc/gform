...
object FormatParser {
  ...
  lazy val expr: Parser[FormatExpr] = (dateFormat
    | textFormat
    | anyWordExpression)

  lazy val dateFormat: Parser[DateFormat] = (anyDateConstraint ^^ ((loc, constraints) => DateFormat(constraints))
    | dateConstraints ^^ ((loc, constraints) => DateFormat(constraints)))

  lazy val dateConstraints: Parser[DateConstraints] = (dateConstraint ~ "," ~ dateConstraints ^^ { (loc, x, _, xs) =>
  ...
  lazy val dateConstraint: Parser[DateConstraint] = (beforeOrAfter ~
    dateExpr ~
    offsetExpression ^^ { (loc, beforeOrAfter, dateExpr, offset) =>
    DateConstraint(beforeOrAfter, dateExpr, offset)
  }
    |
      beforeOrAfter ~
        dateExpr ^^ { (loc, beforeOrAfter, dateExpr) =>
  ...
  lazy val concreteDate: Parser[ConcreteDate] = (yearParser ~ monthDay ^^ { (loc, year, month, day) =>
  ...
  lazy val beforeOrAfter: Parser[BeforeOrAfter] = ("after" ^^ { (loc, after) =>
    After
  }
    | "before" ^^ { (loc, before) =>
      Before
  ...
  lazy val dateExpr: Parser[DateConstraintInfo] = ("today" ^^ { (loc, today) =>
    Today
  }
    | concreteDate
  //    | nextDate
  ...
  lazy val textFormat: Parser[FormatExpr] = (numberFormat
  ...
  lazy val countryCodeFormat: Parser[TextFormat] = ("countryCode" ^^ { (loc, _) =>
  ...
  lazy val basicFormat: Parser[TextFormat] = ("shortText" ^^ { (loc, _) =>
  ...
  lazy val contactFormat: Parser[TextFormat] = ("telephoneNumber" ^^ { (loc, _) =>
  ...
  lazy val numberFormat: Parser[TextFormat] = ("number" ~ numberArgs ^^ { (loc, _, na) =>
  ...
  lazy val positiveNumberFormat: Parser[TextFormat] = ("positiveNumber" ~ numberArgs ^^ { (loc, _, na) =>
  ...
  lazy val governmentIdFormat: Parser[TextFormat] = ("utr" ^^ { (loc, _) =>
    TextFormat(UTR)
  }
    | "nino" ^^ { (loc, _) =>
      TextFormat(NINO)
  ...
  lazy val moneyFormat: Parser[TextFormat] = ("sterling" ^^ { (loc, _) =>
    TextFormat(Sterling)
  }
  ...
  lazy val numberArgs: Parser[(Int, Int, Option[String])] = (wholeFractional ~ "," ~ quotedString ~ ")" ^^ {
    (loc, wf, _, q, _) =>
      (wf.w, wf.f, Some(q))
  }
