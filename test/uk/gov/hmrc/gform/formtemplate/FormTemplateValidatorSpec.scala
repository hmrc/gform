...
import uk.gov.hmrc.gform.sharedmodel.ExampleData.{ ackFormComponent, decFormComponent }
import uk.gov.hmrc.gform.sharedmodel._
import uk.gov.hmrc.gform.sharedmodel.formtemplate.InternalLink.PageLink
import uk.gov.hmrc.gform.sharedmodel.formtemplate.destinations.{ Destinations, PrintSection }
import uk.gov.hmrc.gform.sharedmodel.formtemplate.{ AnyDate, BulletedList, CalendarDate, Checkbox, Choice, ChoicesAvailable, ChoicesSelected, Concat, Constant, DataRetrieveCtx, DataRetrieveDateCtx, Date, DateAfter, DateBefore, DateCtx, DateExprWithOffset, DateFormCtxVar, DateFunction, DateProjection, DateValueExpr, DisplayAsEntered, Dynamic, Else, EmailParameter, Equals, Expr, ExprWithPath, FormComponent, FormComponentId, FormComponentValidator, FormCtx, FormStartDateExprValue, FormTemplate, GreaterThan, HideZeroDecimals, Horizontal, IfElse, IncludeIf, IndexOf, IndexOfDataRetrieveCtx, InformationMessage, Instruction, IsTrue, LeafExpr, LinkCtx, LookupColumn, Mandatory, Not, NumberedList, Offset, OffsetUnit, OffsetYMD, OptionData, OptionDataValue, Page, PageId, PostcodeLookup, Radio, Section, ShortText, StandardInfo, SummariseGroupAsGrid, TaxPeriodDate, TemplatePath, Text, TextArea, TextWithRestrictions, TodayDateExprValue, TypeAhead, ValidIf, Value, Vertical }
...
class FormTemplateValidatorSpec
  ...
  it should {
    "validate and report non-existent email parameter references" in {
      val table = Table(
        ("sections", "emailParameters", "expected"),
        (
          List(
            mkSectionNonRepeatingPage(
              name = "page1",
          ...
          Some(
            NonEmptyList.of(
              EmailParameter("fullName", FormCtx(FormComponentId("fieldNotInFormTemplate")))
            )
          ),
          Invalid("emailParameters.value: fieldNotInFormTemplate doesn't exist in the form")
        ),
        (
          List(
            mkSectionNonRepeatingPage(
          ...
          Some(
            NonEmptyList.of(
              EmailParameter(
                "fullName",
                Else(
                  FormCtx(FormComponentId("directorFullName")),
                  Else(
                    FormCtx(FormComponentId("chairmanFullName")),
                    FormCtx(FormComponentId("ceoFullName"))
                  )
                )
              )
            )
          ),
          Invalid("emailParameters.value: ceoFullName doesn't exist in the form")
        ),
        (
          List(
            mkSectionNonRepeatingPage(
              ...
              EmailParameter(
                ...
                Concat(
                  List(
                    FormCtx(FormComponentId("firstName")),
                    Constant(" "),
                    FormCtx(FormComponentId("lastNames"))
                  )
                )
              )
            )
          ),
          Invalid("emailParameters.value: lastNames doesn't exist in the form")
        ),
        (
          List(
            mkSectionNonRepeatingPage(
          ...
          Some(
            NonEmptyList.of(
              EmailParameter("acknowledgementField", FormCtx(ackFormComponent.head.id))
            )
          ),
          Invalid("emailParameters.value: fieldInAcknowledgementSections doesn't exist in the form")
        ),
        (
          List(
            mkSectionNonRepeatingPage(
          ...
          Some(
            NonEmptyList.of(
              EmailParameter("declarationField", FormCtx(decFormComponent.head.id))
            )
          ),
          Invalid("emailParameters.value: fieldInDeclarationSections doesn't exist in the form")
        )
      )
      forAll(table) { (sections, emailParameters, expected) =>
        val formTemplate = mkFormTemplate(sections, emailParameters)
        val allExprs: List[ExprWithPath] = LeafExpr(TemplatePath.root, formTemplate)
        FormTemplateValidator.validateInvalidReferences(
          mkFormTemplate(sections),
          allExprs,
