...
class FormServiceSpec extends Spec with TypeclassFixtures {
  ...
  val plainFormTemplate = FormTemplate(Some("schemaId"), FormTypeId(""), "formName", "version", "description", "characterSet", DmsSubmission("customerId", "classificationType", "businessArea"), "submitSuccessUrl", "submitErrorUrl", List.empty[Section])
  ...
  val businessDetailsSection = Section(
    ...
    List(
      ...
      FieldValue(FieldId("accountingPeriodStartDate"), Text(Constant(""), total = false), "Accounting period start date", None, mandatory = true, editable = true, submissible = true),
      FieldValue(FieldId("accountingPeriodEndDate"), Text(Constant(""), total = false), "Accounting period end date", None, mandatory = true, editable = true, submissible = true)
    )
  )
