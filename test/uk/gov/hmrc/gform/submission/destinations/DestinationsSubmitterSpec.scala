...
import uk.gov.hmrc.gform.sharedmodel.{ FrontEndSubmissionDesIncludeIfEval, FrontEndSubmissionVariables, LangADT, PdfHtml, SubmissionRef }
...
class DestinationsSubmitterSpec
  ...
  private def exampleTemplateWithDestinations(destination1: Destination, moreDestinations: Destination*): FormTemplate =
    formTemplate.copy(
      destinations =
        Destinations.DestinationList(NonEmptyList.of(destination1, moreDestinations: _*), ackSection, Some(decSection))
    )
    ...
    def expectDestinationSubmitterSubmitIfIncludeIf(
      ...
      (destinationSubmitter
        .submitIfIncludeIf(
          ...
          _: Option[FormData],
          _: LangADT,
          _: FrontEndSubmissionDesIncludeIfEval
        )(_: HeaderCarrier))
        .expects(where {
          (
            dest: Destination,
