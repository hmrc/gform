...
trait ObligationDetailGen {
  def obligationDetailGen: Gen[ObligationDetail] =
    for {
      status                        <- PrimitiveGen.nonEmptyAlphaNumStrGen
      inboundCorrespondenceFromDate <- PrimitiveGen.localDateGen
