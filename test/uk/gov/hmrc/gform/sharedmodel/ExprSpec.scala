...
class ExprSpec extends Spec {
  ...
  "FormCtx.simpleDollarWrites" should "write JSON in the correct format" in {
    val res: JsValue = FormCtx.simpleDollarWrites.writes(formCtx)
    res should be(JsString(s"$${form}"))
  }

  "FormCtx.simpleDollarReads" should "read JSON in the correct format" in {
    val res = FormCtx.simpleDollarReads.reads(JsString(s"$${xyz}"))
    res should be(JsSuccess(FormCtx(s"xyz")))
  }
