...
class FormComponentRejectSpec extends Spec with TableDrivenPropertyChecks {
  ...
  it should "reject syntactically valid templates violating specification constraints" in {
    val table = Table(
      ...
      ("confirming-invalid-page-id",                "sections.confirmation.redirects.if: 'contentInfo' is an info, table or miniSummaryList field and cannot be used in an expression"),
