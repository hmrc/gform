...
class FormComponentRejectSpec extends Spec with TableDrivenPropertyChecks {
  ...
  it should "reject syntactically valid templates violating specification constraints" in {
    val table = Table(
      ...
      ("confirming-invalid-page-id",                "Invalid confirmation: 'confirmationQuestion'. No confirmation pageId: bankAccountPage1 found."),
  ...
  it should "reject template containing invalid reference field" in {
    val table = Table(
      // format: off
      ("json",                                      "expectedMessage"),
      ("invalid-count-usage",                       "sections.fields.[id=lastInfo].infoText: textA cannot be use with .count function. Only MultiFile upload id, AddToList id or Group id can be used with .count"),
